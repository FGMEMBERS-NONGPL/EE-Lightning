...
var ELECTRICAL_UPDATE	= 0.1;									# Update interval in seconds. Set to 0 for once/frame
...
var electrical_update = func {
  ...
  foreach(var supplier; suppliers) {
	...
	if (kind == "alternator" or kind=="generator") {
      ...
      if (kind == "alternator") {
	     var ideal_freq = components[supplier].getNode("frequency").getValue();
		 freq = ideal_freq;
		}
	  if (min == 0 or source_val >= min) {					# Alternator has good volts if source is up to speed
		 ...
		 if (kind == "alternator") {
		     freq = source_val / min * ideal_freq;
			}
      }
	  setprop(components[supplier].getNode("freq-prop").getValue(),freq);
    }
...
var electrical_init = func {
  ...
  for(var i=0; i<size(connector_list); i+=1) {
    ...
    if (!contains(components,output)) {
      print("Error: no match for connector ",i," output in component list");
      continue;									# Skip connector if output matches no component
    }
    if (input == output) {
      print("Error: connector ",i," tried to tie to itself");
      continue;									# Skip connector if output matches input
    }
    if (components[output].getNode("kind").getValue() != "output") {
      print("Error: connector ",i," tried to tie to a source");
...
var ground_power = func {
     var elec = props.globals.getNode("/systems/electrical");
	 var bbvolts = elec.getNode("outputs/busbars/port-busbar").getValue();
	 var gpvolts = elec.getNode("suppliers/ground-power-200V").getValue();
	 
	 var breakers = props.globals.getNode("controls/electrical/circuit-breakers");
	 var gpmv = breakers.getNode("ground-power-mv");	 
	 
	 if (( bbvolts < 5 ) and ( gpvolts > 185 )) {
	     gpmv.setBoolValue(1);
		}
