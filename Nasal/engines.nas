...
print("Initialising Yasim Engines");
...
var Jet =
{
	...
	new: func(n, running = 0, idle_throttle = 0.01, max_start_n1 = 5.21, start_threshold = 3, spool_time = 4, start_time = 0.02, shutdown_time = 4,) 
	{
		...
		m.overspeed_rpm = 121; # 111
		m.failure_prob = 0;
		m.crashed = props.globals.getNode("/sim/crashed");
		...
		m.reheat = props.globals.getNode("engines/engine[" ~ n ~ "]/reheat",1 );
		m.reheat.setValue(0);
	...
	update: func {
        ...
        me.smoking.setValue( ( me.damage.getValue() * 0.55 ) + ( me.explosion.getValue() * 2 ) + ( me.fire.getValue() * 0.6 ) );
		...
		elsif (me.running.getBoolValue())
		{
			...
			me.state.setIntValue(2);
			     ...
			     me.reheat.setValue(0);
				 me.state.setIntValue(3);   
				 interpolate(me.rpm,0,1);
